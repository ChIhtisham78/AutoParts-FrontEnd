...
export default function OrderDetailsPage() {
  ...
  const ChangeStatus = ({ order_status }: FormValues) => {
    ...
    updateOrder({
      ...
      orderStatus: order_status?.status as string,
    });
  };
  ...
  const totalItem = order?.orderLines?.reduce(
    // @ts-ignore
  ...
  async function handleDownloadInvoice() {
    ...
    if (data) {
      ...
      a.href = url;
      a.download = 'order-invoice';
      a.setAttribute('download', 'order-invoice.pdf'); // Ensure you add the file extension

      // Append the link to the document body (not necessary but can be done for Firefox)
      ...
      document.body.removeChild(a); // If you appended the link
    }
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
          <div className="mb-10 w-full sm:mb-0 sm:w-1/2 sm:pe-8">
            ...
            <div className="flex flex-col items-start space-y-1 text-sm text-body">
              ...
              <span>
                {`${t('text-payment-method')}:  ${order?.paymentGateway}`}
              </span>
            </div>
          </div>
          ...
          <div className="w-full sm:w-1/2 sm:ps-8">
            ...
            <div className="flex flex-col items-start space-y-1 text-sm text-body text-start sm:items-end sm:text-end">
              ...
              {order?.shippingsDto && (
                <span>
                  {formatAddress(order.shippingsDto.shippingAddressDto)}
                </span>
              )}
              {order?.customer_contact && <span>{phoneNumber}</span>}
            </div>
