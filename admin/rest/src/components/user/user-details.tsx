...
const UserDetails: React.FC = () => {
  ...
  const { role, id } = getAuthCredentials();
  // const { isLoading: loading } = useMeQuery();
  const { data, isLoading: loading } = useUserQuery({ id: id!?.toString() });
  console.log('vendor data', data);

  const phoneNumber = useFormatPhoneNumber({
    customer_contact: data?.result.phoneNumber as string,
  });
      ...
      <div className="flex flex-wrap gap-3">
        <div
          className={classNames(
            ...
            data?.result.profile ? '' : 'flex bg-[#2B2C2E] text-[#F0F0F0]',
        ...
        >
          <div
            className={classNames(
              ...
              data?.result.profile ? 'h-full w-full' : 'mx-auto self-end',
            )}
          >
            {data?.result.profile ? (
              <Image
          ...
          <div
            className={classNames(
              ...
              data?.result.isActive ? 'text-accent' : 'text-red-500',
            )}
          >
            <CheckMarkFillNew />
          </div>
