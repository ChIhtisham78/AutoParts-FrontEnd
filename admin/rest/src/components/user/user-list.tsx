  ...
  const columns = [
    ...
    },
    {
      ...
      render: (
        name: string,
        {
          ...
          email,
          userName,
        }: { profile: any; email: string; userName: string },
      ) => (
      ...
      render: (permissions: any) => {
        return (
          <div className="flex flex-wrap gap-1.5 whitespace-nowrap">
            {permissions?.map((name: string, index: number) => (
    ...
    // },
    {
      ...
      render: (isActive: boolean) => (
        <Badge
          textKey={isActive ? 'common:text-active' : 'common:text-inactive'}
          color={
            isActive
    ...
    },
    {
      ...
      align: 'right',
      width: 120,
      render: function Render(id: string, { isActive }: any) {
        const { id: myId } = getAuthCredentials();
        const { data } = useUserQuery({ id: myId!?.toString() });
