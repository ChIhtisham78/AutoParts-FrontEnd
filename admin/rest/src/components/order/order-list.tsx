...
const OrderList = ({
  ...
  const columns = [
      ...
      render: (customer: any) => (
        <div className="flex items-center">
          ...
          <div className="flex flex-col whitespace-nowrap font-medium ms-2">
            {customer?.userName ? customer?.userName : t('common:text-guest')}
    ...
    },
    {
      ...
      key: 'orderLines',
      align: 'center',
      render: (orderLines: Product) => <span>{orderLines?.length}</span>,
    },
    {
    ...
    },
    {
      title: (
        <TitleWithSort
          ...
          ascending={
            sortingObj?.sort === SortOrder?.Asc &&
            sortingObj?.column === 'totalAmount'
          }
          isActive={sortingObj?.column === 'totalAmount'}
          className="cursor-pointer"
        />
      ),
      ...
      align: 'center',
      width: 120,
      onHeaderCell: () => onHeaderClick('totalAmount'),
      render: function Render(value: any) {
        const { price } = usePrice({
          amount: value,
        });
