...
// Mapping updated icons with labels
...
export default function CreateOrUpdateCategoriesForm({
  ...
  // Added image watch to handle the disabled state of the button
  const {
    register,
    handleSubmit,
    control,
  ...
  // Watch the 'image' field to check if an image has been uploaded
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const input = {
      ...
      size: values.size.name,
      imageDto: {
        thumbnail: values?.image?.message,
        original: values?.image?.message,
        id: values?.image?.id,
      },
      icon: values.image?.message,
    };
      ...
      <StickyFooterPanel className="z-0">
        <div className="text-end">
          ...
          <Button
            ...
            // Disable button if image hasn't been uploaded
            disabled={
              creating || updating || !imageValue || !imageValue?.message
            }
