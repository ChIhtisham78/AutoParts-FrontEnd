    ...
    >
      ...
      >
        <Image
          ...
          src={
            isValidUrl(shop?.coverImage) ? shop?.coverImage : '/topographic.svg'
          }
          // src={'/topographic.svg'}
          // fill
          width={350}
        ...
        <div className="relative max-w-[calc(100%-104px)] flex-auto pr-4 pt-2">
          ...
          <div className="mt-2 flex w-11/12 items-center gap-1 text-xs leading-none">
            ...
            <p className="truncate text-base-dark">
              {formatAddress(shop?.shopAddressDto as UserAddress)
                ? formatAddress(shop?.shopAddressDto as UserAddress)
                : '???'}
            </p>
          </div>
      ...
      <ul className="mt-4 grid grid-cols-4 divide-x divide-[#E7E7E7] px-2 pb-7 text-center">
        <li>
          <ListItem
            ...
            info={shop?.balanceDto?.adminCommissionRate ?? (0 as number)}
          />
        </li>
        <li>
          <ListItem
            title={t('text-title-sale')}
            info={shop?.balanceDto?.totalEarnings as number}
          />
        </li>
        <li>
          <ListItem
            title={t('text-title-balance')}
            info={shop?.balanceDto?.currentBalance as number}
          />
        </li>
        <li>
          <ListItem title={'Products'} info={shop?.productCount as number} />
        </li>
      </ul>
    </Link>
  );
