...
const ShopForm = ({ initialValues }: { initialValues?: any }) => {
    ...
    ...(initialValues
      ? {
          defaultValues: {
            ...
            logo: {
              message: initialValues.logo,
              id: 1,
              original: initialValues.logo,
              thumbnail: initialValues.logo,
            },
            coverImage: {
              original: initialValues.coverImage,
              thumbnail: initialValues.coverImage,
              message: initialValues.coverImage,
              id: 1,
            },
            balance: {
              payment_info: {
                bank: initialValues.balanceDto?.bankName,
                name: initialValues.balanceDto?.accountHolderName,
                email: initialValues.balanceDto?.accountHolderEmail,
                account: initialValues.balanceDto?.accountNumber,
              },
            },

            address: {
              city: initialValues.shopAddressDto?.city,
              country: initialValues.shopAddressDto?.country,
              zip: initialValues.shopAddressDto?.zip,
              street_address: initialValues.shopAddressDto?.streetAddress,
              state: initialValues.shopAddressDto?.state,
            },

            settings: {
              ...initialValues?.settingDto,
  ...
  async function onSubmit(values: FormValues) {
    ...
    const formattedValues: { [key: string]: any } = {
      ...
      certificateUrl: values.certificateUrl?.message || '',
      settingDto: {
        contact: values.settings.contact || '',
        locationCountry: values.address.country || '',
        locationLat: values.settings.location?.lat || 0,
        locationLng: values.settings.location?.lng || 0,
        locationFormattedAddress:
          values.address.street_address + ',' + values.address.city || '',
        locationZip: values.address.zip || '',
        locationState: values.address.state || '',
        locationCity: values.address.city || '',
      ...
      shopAddressDto: {
        ...
        streetAddress: values.address.street_address || '',
      },
      balanceDto: {
        id: values.balance.id || 0,
        ...
        currentBalance: 0,
        withDrawnAmount: 0,
        accountHolderEmail: values.balance.payment_info?.email || '',
        accountHolderName: values.balance.payment_info?.name || '',
        accountNumber: values.balance.payment_info?.account || '',
        bankName: values.balance.payment_info?.bank || '',
      },
      imageDto: {
        coverImage: values.coverImage?.message,
        logo: values.logo?.message,
      },
      coverImage: values.coverImage?.message,
      logo: values.logo?.message,
      socialDto: {
        id: 0,
        icon: 'facebook',
        link: values.settings.socials?.[0]?.url || '',
        type: '',
        url: '',
      },
    };

    // console.log('settingDto', settingDto);
    if (initialValues) {
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
          <Description
            title={t('Upload your LLC Certificate')}
            className="w-full px-0 pb-5 sm:w-4/12 sm:py-8 sm:pe-4 md:w-1/3 md:pe-5"
          />
