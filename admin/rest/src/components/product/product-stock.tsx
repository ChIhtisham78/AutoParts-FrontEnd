      ...
      render: (_: string, item: Product) => (
        <div className="flex items-center font-medium">
          <div className="relative aspect-square h-9 w-9 shrink-0 overflow-hidden rounded-full border border-border-200/80 bg-gray-100 me-2">
            <Image
              alt={item.name}
              src={item.imageDto?.original ?? siteSettings.product.placeholder}
              fill
              priority={true}
      ...
      render: (shopDto: Shop) => (
        <div className="flex items-center">
          <div className="relative aspect-square h-9 w-9 shrink-0 overflow-hidden rounded-full border border-border-200/80 bg-gray-100 me-2">
            <Image
              src={
                shopDto?.imageDto?.original ?? siteSettings.product.placeholder
              }
              alt={shopDto?.name ?? 'Shop Name'}
              fill
              priority={true}
              sizes="(max-width: 768px) 100vw"
            />
          </div>
          <span className="truncate">{shopDto?.name}</span>
        </div>
      ),
    },
    {
