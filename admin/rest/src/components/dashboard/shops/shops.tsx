...
import { useShopByOwnerQuery } from '@/data/shop';

const ShopList = () => {
  const { t } = useTranslation();
  // const { data, isLoading: loading, error } = useMeQuery();
  const { data, isLoading: loading, error } = useShopByOwnerQuery();
  console.log('data shop', data?.result);
    ...
    <>
      ...
      {!isEmpty(data?.result) ? (
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 2xl:grid-cols-3 3xl:grid-cols-4">
          {data?.result?.map((myShop: any, idx: number) => (
