...
import { createVendorStripe, getStripeVendor } from '@/data/stripe';
import { stripeClient } from '@/data/client/stripe';
...
const OwnerShopLayout = () => {
  ...
  const { permissions, id } = getAuthCredentials();
  // const stripeVendorId = 'acct_1PQiPTRriyJykoz1';
  const { data: vendorData } = useUserQuery({ id });
  const stripeVendorId = vendorData?.result?.stripeVendorId;
  const stripeDashboardAccess = vendorData?.result?.stripeDashboardAccess;
  ...
  } = useProductByCategoryQuery({ limit: 10, language: locale, vendorId: id });

  const {
    data: topRatedProducts,
    isLoading: topRatedProductsLoading,
    error: topRatedProductsError,
  } = useTopRatedProductsQuery({ limit: 10, language: locale, vendorId: id });

  const { data: stripeStatus } = getStripeVendor({ stripeId: stripeVendorId });

  const handleStipeRegistration = () => {
    stripeClient
      .createStripeVendor({ userId: id })
      .then((response) => {
        router.push(response.result.accountLinkUrl);
      })
      .catch((error) => {
        console.error('Error fetching vendor status:', error);
      });
    // router.push(data.accountLinkUrl);
  };

  const handleManageStripe = () => {
    console.log('Redirecting to:', stripeDashboardAccess);
    // window.location.href = stripeDashboardAccess;
  };
  ...
  return (
    <>
      <div className="mb-8 rounded-lg bg-light p-5 md:p-8">
        <PageHeading title={'Stripe Status'} />

        {!stripeVendorId ? (
          <div className="flex flex-row justify-between flex-wrap">
            <h1>You are not registered at stripe</h1>

            <Button
              className=""
              onClick={handleStipeRegistration}
              size={'small'}
          ...
          <div className="flex flex-row flex-wrap justify-between">
            <div>
              <div className="flex flex-row flex-wrap space-x-5">
                <h1 className="text-cyan-700 ">Charges Enabled: </h1>
                <h3>{stripeStatus?.chargesEnabled ? 'True' : 'False'}</h3>
              </div>
              <div className="flex flex-row flex-wrap space-x-5">
                <h1 className="text-cyan-700 ">Payouts Enabled: </h1>
                <h3>{stripeStatus?.payoutsEnabled ? 'True' : 'False'}</h3>
              </div>
            </div>
