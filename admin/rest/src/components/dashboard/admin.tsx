...
export default function Dashboard() {
  ...
  const { data: analytics, isLoading: loading } = useAnalyticsQuery(0);
  ...
  const { data: popularProducts, error: popularProductError } =
    usePopularProductsQuery({ limit: 10, language: locale });
  const popularProductData = popularProducts?.result;
  ...
  const topRatedProducts = topRatedProduct?.result;
  ...
  // const lowStockProduct = popularProductData?.map((product: any) => {
  //   if (product.quantity > 0 && product.id > 0 && product.quantity < 10) {
  //     return {
  //       ...product,
  //       quantity: 0,
  //     };
  ...
  const productByCategory = productsByCategory?.result;
  ...
  if (loading || orderLoading || topRatedProductsLoading) {
  ...
  return (
    <div className="grid gap-7 md:gap-8 lg:grid-cols-2 2xl:grid-cols-12">
      ...
      <LowStockProduct
        //@ts-ignore
        products={lowStockProduct?.result}
