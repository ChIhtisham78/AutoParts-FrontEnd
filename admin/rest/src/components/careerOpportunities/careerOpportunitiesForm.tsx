...
const CareerOpportunitiesForm = () => {
  const { t } = useTranslation();
  const router = useRouter();
  const { alignLeft } = useIsRTL();

  // Local state for homePages
  ...
  const handleDeleteImage = (id: number) => {
    deletehomepage(id, {
      onSuccess: () => {
        setHomePages((prev) => prev.filter((page) => page.id !== id)); // Update local state
      },
    });
  };
    ...
    },
    {
      ...
      render: (id: number, rowData: HomePage) => (
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
          <button
            onClick={() => router.push(`/home-pages/${rowData.id}/edit`)}
            title="Edit"
            style={{
              border: 'none',
