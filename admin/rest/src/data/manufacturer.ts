...
import { engineClient } from './client/engine';
...
export const useManufacturerQuery = ({ slug, language }: GetParams) => {
  const { data, error, isLoading } = useQuery<Manufacturer, Error>(
    ...
    () => manufacturerClient.getBySlug({ slug, language }),
  );
...
export const useManufacturersQuery = (
  ...
  const { data, error, isLoading } = useQuery<ManufacturerPaginator, Error>(
    ['api/manufacturers/manufacturers', options],
    ({ queryKey, pageParam }) =>
...
export const useModelsQuery = (options: Partial<ManufacturerQueryOptions>) => {
  const { data, error, isLoading } = useQuery<ManufacturerPaginator, Error>(
    ['api/ManufactureModel/GetManufactureModel', options],
    ({ queryKey, pageParam }) =>
...
export const useEnginesQuery = (options: Partial<ManufacturerQueryOptions>) => {
  const { data, error, isLoading } = useQuery<ManufacturerPaginator, Error>(
    ['api/engine/getEngines', options],
    ({ queryKey, pageParam }) =>
      engineClient.paginated(Object.assign({}, queryKey[1], pageParam)),
    {
      keepPreviousData: true,
    },
  );
