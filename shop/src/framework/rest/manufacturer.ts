...
export function useManufacturers(options?: ManufacturerQueryOptions) {
  ...
  return {
    manufacturers: data?.pages?.flatMap((page) => page.result) ?? [],
...
export function useMake(options?: ManufacturerQueryOptions) {
  const { locale, query } = useRouter();
  ...
  const {
    ...
    [API_ENDPOINTS.MANUFACTURERS, formattedOptions],
    ({ queryKey, pageParam }) =>
      client.make.all(Object.assign({}, queryKey[1], pageParam)),
    {
      getNextPageParam: ({ current_page, last_page }) =>
        last_page > current_page && { page: current_page + 1 },
    },
  ...
  return {
    make: data?.pages?.flatMap((page) => page.result) ?? [],
    paginatorInfo: Array.isArray(data?.pages)
      ? mapPaginatorData(data?.pages[data.pages.length - 1])
      : null,
    isLoading,
...
export function useEngines(options?: ManufacturerQueryOptions) {
  const { locale, query } = useRouter();
  ...
  const {
    ...
    [API_ENDPOINTS.MANUFACTURERS, formattedOptions],
    ({ queryKey, pageParam }) =>
      client.engine.all(Object.assign({}, queryKey[1], pageParam)),
    {
      getNextPageParam: ({ current_page, last_page }) =>
        last_page > current_page && { page: current_page + 1 },
    },
  ...
  return {
    engines: data?.pages?.flatMap((page) => page.result) ?? [],
    paginatorInfo: Array.isArray(data?.pages)
      ? mapPaginatorData(data?.pages[data.pages.length - 1])
      : null,
    isLoading,
