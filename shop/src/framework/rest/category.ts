...
export function useCategories(options?: Partial<CategoryQueryOptions>) {
  ...
  const {
    ...
    ['api/category/category', formattedOptions],
    ({ queryKey, pageParam }) =>
  ...
  return {
    categories: data?.pages?.flatMap((page) => page.result) ?? [],
...
export function useSubCategories(options?: Partial<CategoryQueryOptions>) {
  ...
  const {
    ...
    ['api/subcategorylist/', formattedOptions],
    ({ queryKey, pageParam }) =>
      client.subcategories.all(Object.assign({}, queryKey[1], pageParam)),
    {
      getNextPageParam: ({ current_page, last_page }) =>
        last_page > current_page && { page: current_page + 1 },
    },
  ...
  return {
    subCategories: data?.pages?.flatMap((page) => page.result) ?? [],
    paginatorInfo: Array.isArray(data?.pages)
      ? mapPaginatorData(data?.pages[data.pages.length - 1])
      : null,
    isLoading,
