...
export function useOrders(options?: Partial<OrderQueryOptions>) {
  ...
  return {
    orders: data?.pages?.flatMap((page) => page.result) ?? [],
...
export function useOrder({ id }: { id: any }) {
  const { data, isLoading, error, isFetching, refetch } = useQuery<
    Order,
    Error
  >(['order', id], () => client.orders.get(id), {
    refetchOnWindowFocus: false,
  });
...
export function useCreateOrder() {
  ...
  const { mutate: createOrder, isLoading } = useMutation(client.orders.create, {
    onSuccess: (data) => {
      const { trackingNumber, payment_gateway, payment_intent, id } =
        data?.result;
      console.log(
        trackingNumber,
        payment_gateway,
      ...
      if (id) {
        if (
          ...
          return router.push(Routes.order(id));
        }
        ...
        if (payment_intent?.payment_intent_info?.is_redirect) {
          ...
          return router.push(`${Routes.order(id)}/payment`);
        }
      }
    },
    onError: (error) => {
  ...
  return useMutation(client.orders.verify, {
    onSuccess: (data) => {
      ...
      if (data?.errors as string) {
        ...
        // FIXME
        //@ts-ignore
        setVerifiedResponse(data.result);
      }
    },
    onError: (error) => {
      const {
