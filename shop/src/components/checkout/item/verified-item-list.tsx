...
const VerifiedItemList: React.FC<Props> = ({ className }) => {
  ...
  const available_items = items?.filter(
    (item) => !verifiedResponse?.unavailableProducts?.includes(item.id),
  );
  ...
  const { price: tax } = usePrice(
    verifiedResponse && {
      amount: verifiedResponse.totalTax ?? 0,
    },
  );

  const { price: shipping } = usePrice(
    verifiedResponse && {
      amount: verifiedResponse.shippingCharge ?? 0,
    },
  );
  ...
  console.log(base_amount, verifiedResponse);
  const totalPrice = verifiedResponse
    ? calculatePaidTotal(
        {
          totalAmount: base_amount,
          tax: verifiedResponse?.totalTax,
          shipping_charge: freeShippings ? 0 : verifiedResponse?.shippingCharge,
        },
